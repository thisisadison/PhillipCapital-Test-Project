import { STALE_AFTER_DAYS, TIMEZONE } from "@/server/config";
import type { Digest, DigestRun } from "@/server/domain/digest";
import { daysBetween, describeAge, formatEditionDate, formatTimestamp, toDateKey } from "./dates";
 ...
 *
 * The rule: an edition is shown as current only when the most recent pipeline
 * attempt actually succeeded and produced it. If a run failed afterwards, or
 * none has happened in longer than the cadence allows, the page says so above
 * the content. The generation date is on the page unconditionally; this adds
 * the interpretation of it.
 *
 * Pure, so the failure modes are testable without a filesystem or an API key.
 */
export function deriveStatus(
  ...
  if (!digest) return { health: "empty", ageDays: null, notice: emptyNotice(lastRun) };

  const ageDays = Math.max(0, daysBetween(digest.date, toDateKey(now, TIMEZONE)));

  // The case that matters most: the content on screen is real, but it is not
  // this week's.
  if (lastRun?.status === "failed" && lastRun.finishedAt > digest.generatedAt) {
    return {
      ...
      notice: {
        ...
        detail:
          `An attempt to refresh the digest on ${formatTimestamp(lastRun.finishedAt, TIMEZONE)} did ` +
          `not complete. You are reading the edition of ${formatEditionDate(digest.date)}, generated ` +
...
function emptyNotice(lastRun: DigestRun | null): StatusNotice {
  if (lastRun?.status === "failed") {
    return {
      ...
      detail:
        `The most recent attempt, on ${formatTimestamp(lastRun.finishedAt, TIMEZONE)}, failed before ` +
        `it produced an edition. Nothing is being hidden from you — there is genuinely nothing to ` +
        `show yet.`,
    };
  }
