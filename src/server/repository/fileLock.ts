 ...
 * across requests: the scheduled job may run as a separate `digest:generate`
 * invocation while the web app is serving a manual trigger, and the
 * manual-trigger rate limit is only meaningful if it sees every run.
 */
export async function withFileLock<T>(lockPath: string, fn: () => Promise<T>): Promise<T> {
  ...
  for (;;) {
    try {
      ...
      if (Date.now() > deadline) throw new Error(`Timed out waiting for lock at ${lockPath}`);
