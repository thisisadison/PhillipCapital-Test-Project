import { DEFAULT_TIMEZONE } from "@/lib/digest/dates";

/**
 * Runtime configuration, read once from the environment.
 *
 * Anything here that a reader of the page could be misled by — the cadence, the
 * staleness threshold — is also stated in the UI, so the page never claims to
 * be current on a promise the configuration does not keep.
 */
...
/**
 * After this many days without a successful run, the current edition is
 * labelled out of date. Set to the cadence plus a couple of days of grace, so a
 * single missed Monday shows up rather than passing unnoticed.
 */
export const STALE_AFTER_DAYS = 10;

/**
 * Minimum gap between manual regenerations. The button exists for demos and for
 * recovering from a failed scheduled run — not as a per-visitor refresh.
 */
